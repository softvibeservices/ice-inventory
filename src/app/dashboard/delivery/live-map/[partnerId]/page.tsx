...
// ✅ COMPLETE FIX - No window errors, removed Center/Auto-Center/Dark buttons
...
// ✅ FIXED: All react-leaflet imports with SSR disabled
...
export default function LiveMapPage() {
  ...
  // ✅ FIXED: Icons created only on client side
  const [L, setL] = useState<any>(null);
  const [pulsingIcon, setPulsingIcon] = useState<any>(null);
  const [startIcon, setStartIcon] = useState<any>(null);

  // ✅ Load Leaflet and create icons on client side only
  useEffect(() => {
    if (typeof window !== 'undefined') {
      import('leaflet').then((leaflet) => {
        ...
        // Create pulsing icon
        const pulsing = leaflet.default.divIcon({
        ...
        setPulsingIcon(pulsing);

        // Create start icon
        const start = leaflet.default.divIcon({
  ...
  // ✅ Create waypoint icon function (only called on client)
  const createWaypointIcon = useCallback((index: number, total: number) => {
  ...
  // ✅ Fit Trail button handler (KEPT - THIS ONE WORKS)
  const handleFitTrailClick = useCallback(() => {
    if (!location?.trail || location.trail.length === 0 || !mapInstance || !L) return;
    
    const bounds = L.latLngBounds(
      location.trail.map((p: LocationPoint) => [p.latitude, p.longitude] as LatLngType)
    );
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 px-4 sm:px-6 py-4 mb-6">
            <div className="flex flex-col gap-4">
              ...
              {/* Controls Row - ✅ REMOVED: Center, Auto-Center, Dark buttons */}
              {location && (
                <div className="flex flex-wrap items-center gap-3">
                  {/* Map Layer Selector - ✅ REMOVED Dark mode */}
                  ...
                  {location.trail && location.trail.length > 0 && (
                    <>
                      ...
                      {/* ✅ KEPT: Fit Trail button (this one works) */}
                      <button
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative">
              ...
              <div className="w-full h-[calc(100vh-380px)] min-h-[500px]">
                ...
                >
                    ...
                    <>
                      {location.trail
                        ...
                        .map((point, index, filteredArray) => {
                          const icon = createWaypointIcon(index, filteredArray.length);
                          return icon ? (
                            <Marker
                              key={`waypoint-${index}`}
                  ...
                  {showTrail && trailCoordinates.length > 1 && startIcon && (
                    <Marker
